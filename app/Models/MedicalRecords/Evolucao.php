...
class Evolucao extends Model
{
    ...
    public function byAgendamento(int $agendamento)
    {
        ...
        $db = $database->prepare("SELECT * FROM ficha_evolucao WHERE agendamento_id = :agendamento");
    ...
    {
        $database = self::getInstance();
        $db = $database->prepare("UPDATE ficha_evolucao SET informacao = :informacao WHERE agendamento_id = :agendamento");
    ...
    {
        $database = self::getInstance();
        $db = $database->prepare("INSERT INTO ficha_evolucao (informacao, agendamento_id) VALUES (:informacao, :agendamento)");
        $db->bindParam(":informacao", $prescricao['informacao']);
        $db->bindParam(":agendamento", $agendamento);
        $db->execute();
