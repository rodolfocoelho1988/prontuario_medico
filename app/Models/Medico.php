...
class Medico extends Model
{
    ...
    {
        $db = self::getInstance()->prepare("SELECT t1.*, t2.nome, t3.nome as nacionalidade FROM medico AS t1 INNER JOIN pessoa AS t2 ON (t1.pessoa_id = t2.id) INNER JOIN pais AS t3 ON (t2.nacionalidade_id = t3.id)");
        $db->execute();
